...
| WhatsApp sessions | ✅ Persistencia + reconexión | `SessionManager` en `whatsapp-core/`, persistencia de sesión en Postgres, reconexión automática con toggle. Sin subsistema de snapshots (eliminado — `auth_data` nunca tuvo filas en producción). Double-init resuelto en `develop`; schema B1 añade `tenant_id`/`ai_agent_id` nullable para futuro |
