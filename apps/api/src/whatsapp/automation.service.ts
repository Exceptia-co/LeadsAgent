...
export class AutomationService {
  ...
  async processIncomingMessage(
    ...
    try {
      ...
      // Process workflows
      await this.processWorkflows('message_received', {
        lead,
        message,
        sessionId,
      });
  ...
  async processStatusChange(
    ...
    try {
      ...
      await this.processStatusChangeResponses(
        lead,
        oldStatus,
        newStatus,
        sessionId,
      ...
      // Process workflows
      await this.processWorkflows('status_changed', {
        lead,
        oldStatus,
        newStatus,
        sessionId,
  ...
  private async processAutoResponses(
    ...
    const autoResponseRules: AutoResponseRule[] = [
      ...
      },
      {
        ...
        response: {
          message:
            '🛠️ Entiendo que necesitas ayuda. No te preocupes, estamos aquí para solucionarlo.\\n\\n¿Podrías describir brevemente el problema que estás experimentando?',
          type: 'text',
          delay: 1,
        },
        isActive: true,
    ...
    const matchingRule = autoResponseRules
      ...
      .sort(
        (a, b) =>
          (a.conditions.priority || 999) - (b.conditions.priority || 999),
      )[0];
    ...
    if (matchingRule) {
      ...
      setTimeout(
        async () => {
          ...
          this.logger.log(
            `Auto response "${matchingRule.name}" sent to lead ${lead.id}`,
          );
...
No te preocupes, estaremos aquí cuando estés listo. ¿Te gustaría recibir información periódica sobre nuestras ofertas?`,
    ...
    if (newStatus in statusMessages && oldStatus !== newStatus) {
      setTimeout(async () => {
        ...
        this.logger.log(
          `Status change message sent to lead ${lead.id} (${oldStatus} -> ${newStatus})`,
        );
      }, 2000);
    }
  }
        ...
        actions: [
          {
            ...
            params: {
              delay: 60, // 1 hour
              message:
                '👋 Hola de nuevo! ¿Has tenido oportunidad de revisar la información que te enviamos?',
            },
          },
        ],
      },
  ...
  private async executeWorkflowActions(
    ...
    for (const action of actions) {
      try {
        ...
        this.logger.error(
          `Error executing workflow action ${action.type}:`,
          error,
        );
      }
    }
  ...
  private async assignToUser(leadId: string, params: any): Promise<void> {
    ...
    this.logger.log(
      `Lead ${leadId} should be assigned to user with role: ${params.role}`,
    );
  ...
  private async updateLeadPriority(
    leadId: string,
    reason: string,
  ): Promise<void> {
    try {
