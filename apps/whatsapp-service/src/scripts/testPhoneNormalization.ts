...
async function runPhoneNormalizationTests(): Promise<void> {
  ...
  for (let i = 0; i < testCases.length; i++) {
    ...
    if (testPassed) {
      ...
      console.log(
        `   ❌ FAILED - Esperado: ${testCase.shouldBeEqual ? 'IGUALES' : 'DIFERENTES'}, Obtenido: ${areEqual ? 'IGUALES' : 'DIFERENTES'}`
      );
    }
  }
  ...
  if (passedTests === totalTests) {
    ...
    logger.warn(
      `⚠️ ${totalTests - passedTests} pruebas fallaron. Revisar la lógica de normalización.`
    );
  }
}
  ...
  try {
    ...
    for (let i = 1; i < testNumbers.length; i++) {
      ...
      try {
        ...
        if (duplicateLead) {
          console.log(
            `   ❌ ERROR: Se creó un lead duplicado (ID: ${duplicateLead.id}) - La normalización falló`
          );
        }
      } catch (error: any) {
    ...
    if (testLeads.length === 1) {
      ...
      console.log(
        `   ❌ ERROR: Se encontraron ${testLeads.length} leads con números equivalentes:`
      );
