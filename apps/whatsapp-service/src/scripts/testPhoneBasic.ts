...
async function testExtensiveNormalization(): Promise<void> {
  ...
  const testCases = [
    // Casos argentinos
    { input: '+5491123456789', expected: '5491123456789', description: 'Número argentino móvil con +' },
    { input: '5491123456789', expected: '5491123456789', description: 'Número argentino móvil sin +' },
    { input: '+549 11 2345 6789', expected: '5491123456789', description: 'Número argentino con espacios' },
    { input: '+54-9-11-2345-6789', expected: '5491123456789', description: 'Número argentino con guiones' },
    { input: '011-2345-6789', expected: '549112345678', description: 'Número local argentino con 0' },
    { input: '11-2345-6789', expected: '549112345678', description: 'Número local argentino sin 0' },
  ...
  console.log(`📈 RESUMEN: ${passed}/${total} tests passed (${(passed/total*100).toFixed(1)}%)`);
  console.log('');
}
...
async function testComplexEquivalence(): Promise<void> {
  ...
  let groupsPassed = 0;
  let totalGroups = equivalenceCases.length;
    ...
    for (let j = 0; j < numbers.length; j++) {
      for (let k = j + 1; k < numbers.length; k++) {
        ...
        if (isEquivalent !== expectedEquivalence) {
          allPassed = false;
          console.log(`    ❌ FAILED - Expected: ${expectedEquivalence ? 'EQUIVALENT' : 'DIFFERENT'}`);
        } else {
          console.log(`    ✅ PASSED`);
        }
      }
  ...
  console.log(`📈 RESUMEN GRUPOS: ${groupsPassed}/${totalGroups} passed (${(groupsPassed/totalGroups*100).toFixed(1)}%)`);
  console.log('');
}
...
async function testPhoneListSearch(): Promise<void> {
  ...
  console.log(`📈 RESUMEN BÚSQUEDA: ${passed}/${total} tests passed (${(passed/total*100).toFixed(1)}%)`);
}
...
async function main(): Promise<void> {
  try {
    ...
    console.log('El sistema de normalización de números telefónicos está funcionando correctamente!');
