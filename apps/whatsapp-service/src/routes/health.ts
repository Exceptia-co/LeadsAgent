        ...
        summary: {
          ...
          healthyPercentage:
            dashboard.overview.totalSessions > 0
              ? Math.round(
                  (dashboard.overview.healthySessions / dashboard.overview.totalSessions) * 100
                )
              : 0,
    ...
    const computedMetrics = {
      healthScore:
        dashboard.overview.totalSessions > 0
          ...
          : 0,
      responseTimeStatus:
        dashboard.overview.averageResponseTime < 2000
          ? 'good'
          : dashboard.overview.averageResponseTime < 5000
            ? 'warning'
            : 'poor',
      alertSeverityDistribution: dashboard.alerts.reduce(
        (acc, alert) => {
...
router.get('/metrics', async (req, res) => {
  try {
    ...
    const responseTimeBuckets = sessionStatuses.reduce(
      (buckets, status) => {
    ...
    const authHealthDistribution = sessionStatuses.reduce(
      (dist, status) => {
    ...
    const metrics = {
      ...
      whatsapp_health_score_percentage:
        dashboard.overview.totalSessions > 0
          ? Math.round(
              (dashboard.overview.healthySessions / dashboard.overview.totalSessions) * 100
            )
