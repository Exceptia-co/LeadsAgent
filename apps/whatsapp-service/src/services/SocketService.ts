...
export class SocketService {
    ...
    whatsappNamespace.on('connection', (socket: Socket) => {
      ...
      socket.on('sessions:get_current_state', async () => {
        try {
          ...
          // Import WhatsApp service to get current sessions
          const WhatsAppServiceModule = await import('./WhatsAppServiceSimple')
      ...
      socket.on('disconnect', (reason) => {
        ...
        logger.info(`🔌 Dashboard client disconnected from WhatsApp sessions: ${clientId} (${reason})`)
  ...
  emitQRCodeUpdate(sessionId: string, qrCode: string): void {
    ...
    logger.debug(`📡 QR code update broadcasted to ${this.connectedClients.size} clients for session: ${sessionId}`)
  }
  ...
  processWebhookEvent(payload: WebhookPayload): void {
    ...
    switch (payload.event) {
      ...
      case 'authenticated':
        logger.info(`✅ Emitting session connected for session ${payload.sessionId} with number ${payload.data.number}`);
      ...
      case 'status_change':
        logger.info(`🔄 Emitting status change for session ${payload.sessionId}: ${payload.data.status}`);
