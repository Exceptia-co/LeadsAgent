...
async function updateKnowledgeBase() {
    try {
        logger_1.logger.info('🚀 Iniciando actualización de knowledge base...');
        // Inicializar tablas
        await DatabaseService_1.default.initializeTable();
        await DatabaseService_1.default.initializeAITrainingTable();
        // Eliminar entradas existentes para evitar duplicados
        logger_1.logger.info('🧹 Limpiando entradas existentes...');
        await DatabaseService_1.default.clearKnowledgeBase();
        // Agregar nuevas entradas
        logger_1.logger.info('📚 Agregando nuevas entradas de knowledge base...');
        for (const entry of knowledgeBaseEntries) {
            await DatabaseService_1.default.addKnowledgeBase(entry);
            logger_1.logger.info(`✅ Agregada entrada: ${entry.title}`);
        }
        logger_1.logger.info('🎉 Knowledge base actualizada exitosamente!');
        logger_1.logger.info(`📊 Total de entradas agregadas: ${knowledgeBaseEntries.length}`);
        // Mostrar estadísticas
        const stats = await DatabaseService_1.default.getKnowledgeBaseStats();
        logger_1.logger.info('📈 Estadísticas de Knowledge Base:', stats);
    }
    catch (error) {
        logger_1.logger.error('❌ Error actualizando knowledge base:', error);
        throw error;
    }
}
// Función principal
