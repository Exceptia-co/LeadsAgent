...
class DatabaseService {
    ...
    initializePool() {
        try {
            if (!process.env.DATABASE_URL) {
                logger_1.logger.warn('DATABASE_URL no está configurado, funcionando sin persistencia de base de datos');
                return;
            }
            ...
            this.pool.on('error', (err) => {
                logger_1.logger.error('Error en el pool de conexiones PostgreSQL:', err);
            });
            logger_1.logger.info('Pool de conexiones PostgreSQL inicializado');
        }
        catch (error) {
            logger_1.logger.error('Error inicializando pool de base de datos:', error);
        }
    }
    ...
    async initializeTable() {
        if (!this.pool) {
            logger_1.logger.warn('No hay conexión a base de datos disponible');
            return;
        }
    ...
    `;
        try {
            ...
            logger_1.logger.info('Tablas whatsapp_conversations y whatsapp_whitelist_logs verificadas/creadas correctamente');
        }
        catch (error) {
            logger_1.logger.error('Error creando tablas:', error);
        }
    }
    // Guardar conversación
    async saveConversation(data) {
        // LOG DETALLADO: Estado inicial
        logger_1.logger.info('🔍 [DIAGNOSTIC] saveConversation called with data:', {
        ...
        if (!this.pool) {
            logger_1.logger.error('❌ [DIAGNOSTIC] No database connection available!');
            logger_1.logger.error('❌ [DIAGNOSTIC] DATABASE_URL:', process.env.DATABASE_URL ? 'SET' : 'NOT SET');
            return null;
        }
        logger_1.logger.info('✅ [DIAGNOSTIC] Database pool connection available');
        const query = `
    ...
    `;
        ...
        logger_1.logger.info('📝 [DIAGNOSTIC] Query values prepared:', {
            sessionId: values[0],
        ...
        try {
            logger_1.logger.info('🔄 [DIAGNOSTIC] Executing database query...');
            const result = await this.pool.query(query, values);
            const conversationId = result.rows[0]?.id;
            logger_1.logger.info('✅ [DIAGNOSTIC] Query executed successfully!', {
                conversationId,
                rowsAffected: result.rowCount,
                returningId: result.rows[0]?.id
            });
            logger_1.logger.info('💾 Conversación guardada correctamente:', {
                id: conversationId,
                phoneNumber: data.phoneNumber,
                sessionId: data.sessionId
            });
            ...
            logger_1.logger.error('❌ [DIAGNOSTIC] Error executing database query:', {
                error: error.message,
                code: error.code,
                detail: error.detail,
                hint: error.hint,
            ...
            logger_1.logger.error('❌ Error guardando conversación (legacy):', error);
            return null;
        }
    }
    ...
    async getConversationHistory(phoneNumber, limit = 50) {
        if (!this.pool) {
            logger_1.logger.warn('No hay conexión a base de datos');
            return [];
        }
        const query = `
    ...
    `;
        try {
            ...
            logger_1.logger.error('Error obteniendo historial de conversación:', error);
            return [];
        }
    }
    // Obtener historial reciente para contexto de IA (últimos N mensajes)
    async getRecentContext(phoneNumber, sessionId, limit = 10) {
        if (!this.pool) {
            return [];
        }
        const query = `
    ...
    `;
        try {
            ...
            logger_1.logger.error('Error obteniendo contexto reciente:', error);
            return [];
        }
    }
    ...
    async searchConversations(searchTerm, sessionId, limit = 20) {
        if (!this.pool) {
            return [];
        }
        let query = `
    ...
    `;
        ...
        try {
            ...
            logger_1.logger.error('Error buscando conversaciones:', error);
            return [];
        }
    }
    ...
    async testConnection() {
        ...
        try {
            ...
            logger_1.logger.info('Conexión a base de datos verificada:', result.rows[0]);
            return true;
        }
        catch (error) {
            logger_1.logger.error('Error verificando conexión a base de datos:', error);
            return false;
        }
    }
    ...
    async getAllLeads() {
        ...
        `;
          ...
          `;
                    ...
                    logger_1.logger.info(`✅ Obtenidos ${realLeads.length} leads de la base de datos`);
                    return realLeads;
                }
            }
            catch (error) {
                logger_1.logger.warn('Error obteniendo leads de la base de datos, usando datos mockeados:', error);
            }
        }
        ...
        logger_1.logger.info(`🔧 Usando ${mockLeads.length} leads mockeados para desarrollo`);
        return mockLeads;
    }
    // Create a new lead
    async createLead(leadData) {
        if (!this.pool) {
            logger_1.logger.warn('No database connection available, cannot create lead');
            return null;
        }
        try {
      ...
      `;
            const tableExists = await this.pool.query(checkTableQuery);
            if (!tableExists.rows[0].exists) {
                logger_1.logger.error('Leads table does not exist');
                return null;
            }
      ...
      `;
            ...
            if (result.rows.length > 0) {
                ...
                logger_1.logger.info(`✅ Created new lead: ${newLead.name || 'Unnamed'} (${newLead.phone})`);
                return newLead;
            }
            logger_1.logger.error('Failed to create lead - no rows returned');
            return null;
        }
        catch (error) {
    ...
    async updateLeadWhatsAppAuth(leadId, whatsappAuthorized) {
        if (this.pool) {
            try {
                const query = `
          UPDATE leads 
        ...
        `;
                ...
                if (result.rows.length > 0) {
                    logger_1.logger.info(`✅ Lead ${leadId} WhatsApp authorization updated to: ${whatsappAuthorized}`);
                    return true;
                }
                else {
                    logger_1.logger.warn(`⚠️ Lead ${leadId} not found for WhatsApp authorization update`);
                    return false;
                }
            }
            catch (error) {
                logger_1.logger.error('Error updating lead WhatsApp authorization:', error);
                return false;
            }
        }
        // For mock data, we can't actually update, so just log and return true
        logger_1.logger.info(`🔧 Mock update: Lead ${leadId} WhatsApp authorization would be set to: ${whatsappAuthorized}`);
        return true;
    }
    ...
    `;
        ...
        try {
            ...
            logger_1.logger.error('Error obteniendo conversaciones recientes:', error);
            return [];
        }
    }
    // Registrar decisión de whitelist en logs
    async logWhitelistDecision(data) {
        if (!this.pool) {
            logger_1.logger.warn('No hay conexión a base de datos, log de whitelist no guardado');
            return null;
        }
        const query = `
    ...
    `;
        ...
        try {
            ...
            logger_1.logger.debug('Whitelist decision logged:', {
                id: logId,
                phoneNumber: data.phoneNumber,
            ...
            logger_1.logger.error('Error logging whitelist decision:', error);
            return null;
        }
    }
    // Obtener logs de whitelist con filtros
    async getWhitelistLogs(options = {}) {
        if (!this.pool) {
            return [];
        }
    ...
    `;
        ...
        try {
            ...
            logger_1.logger.error('Error getting whitelist logs:', error);
            return [];
        }
    }
    // Obtener estadísticas de whitelist
    async getWhitelistStats(options = {}) {
        if (!this.pool) {
            return {
                totalDecisions: 0,
                allowedCount: 0,
    ...
    `;
        ...
        try {
            ...
            logger_1.logger.error('Error getting whitelist statistics:', error);
            return {
                totalDecisions: 0,
                allowedCount: 0,
                blockedCount: 0,
    ...
    async getConversations(limit = 50, offset = 0) {
        if (!this.pool) {
            logger_1.logger.warn('No hay conexión a base de datos para obtener conversaciones');
      ...
      `;
            ...
            logger_1.logger.info(`✅ Obtenidas ${conversations.length} conversaciones de la base de datos`);
            return conversations;
        }
        catch (error) {
            logger_1.logger.error('Error obteniendo conversaciones:', error);
            return this.getMockConversations(limit, offset);
        }
    }
    ...
    async getConversationById(conversationId) {
        try {
            ...
            logger_1.logger.error('Error obteniendo conversación por ID:', error);
            return null;
        }
    }
    ...
    async getConversationMessages(conversationId, limit = 50, offset = 0) {
      ...
      `;
            ...
            logger_1.logger.error('Error obteniendo mensajes de conversación:', error);
            return { conversation: null };
        }
    }
    // Crear o actualizar conversación
    async createOrUpdateConversation(leadId, sessionId) {
        try {
            const leads = await this.getAllLeads();
            const lead = leads.find(l => l.id === leadId);
            if (!lead || !lead.phone) {
                logger_1.logger.warn(`Lead ${leadId} no encontrado o sin teléfono`);
                return null;
            }
            // Crear ID de conversación basado en el número de teléfono
            const conversationId = `conv_${lead.phone.replace(/[^0-9]/g, '')}`;
            logger_1.logger.info(`Conversación ${conversationId} creada/actualizada para lead ${leadId}`);
            return conversationId;
        }
        catch (error) {
            logger_1.logger.error('Error creando/actualizando conversación:', error);
            return null;
        }
    }
    // Datos mockeados para desarrollo
    getMockConversations(limit, offset) {
    ...
    getMockMessages(phoneNumber, limit) {
        const mockMessages = [
            {
                id: 'msg_1',
                content: '¡Hola! Me interesa conocer más sobre sus servicios',
    ...
    async getKnowledgeBase(category) {
        if (!this.pool) {
      ...
      `;
            ...
            logger_1.logger.error('Error obteniendo knowledge base:', error);
            return this.getDefaultKnowledgeBase();
        }
    }
    // Obtener configuración IA
    async getAIConfiguration(key) {
        if (!this.pool) {
      ...
      `;
            ...
            logger_1.logger.error('Error obteniendo configuración IA:', error);
            return this.getDefaultConfig(key);
        }
    }
    // Actualizar configuración IA
    async updateAIConfiguration(key, value, updatedBy) {
        if (!this.pool) {
            logger_1.logger.info(`Mock update: ${key} = ${value.substring(0, 100)}...`);
            return true;
        }
        try {
            const query = `
      ...
      `;
            ...
            logger_1.logger.info(`✅ Configuración IA actualizada: ${key}`);
            return true;
        }
        catch (error) {
            logger_1.logger.error('Error actualizando configuración IA:', error);
            return false;
        }
    }
    ...
    async searchKnowledgeBase(query) {
        if (!this.pool) {
            return this.getDefaultKnowledgeBase().filter(item => item.content.toLowerCase().includes(query.toLowerCase()) ||
                item.title.toLowerCase().includes(query.toLowerCase())).slice(0, 3); // Limitar resultados
        }
        try {
      ...
      `;
            ...
            logger_1.logger.error('Error buscando en knowledge base:', error);
            return [];
        }
    }
    ...
    async getMessageTemplates(category, activeOnly = true) {
        if (!this.pool) {
      ...
      `;
            ...
            logger_1.logger.error('Error obteniendo templates:', error);
            return this.getDefaultTemplates();
        }
    }
    ...
    async createMessageTemplate(data) {
        if (!this.pool) {
            logger_1.logger.info(`Mock template created: ${data.name}`);
      ...
      `;
            ...
            logger_1.logger.info(`✅ Template creado: ${data.name} (${templateId})`);
            return templateId;
        }
        catch (error) {
            logger_1.logger.error('Error creando template:', error);
            return null;
        }
    }
    // Actualizar template
    async updateMessageTemplate(id, updates) {
        if (!this.pool) {
            logger_1.logger.info(`Mock template updated: ${id}`);
            return true;
        }
        try {
            const setParts = [];
      ...
      `;
            const result = await this.pool.query(query, values);
            if (result.rows.length > 0) {
                logger_1.logger.info(`✅ Template actualizado: ${id}`);
                return true;
            }
            else {
                logger_1.logger.warn(`Template no encontrado: ${id}`);
                return false;
            }
        }
        catch (error) {
            logger_1.logger.error('Error actualizando template:', error);
            return false;
        }
    }
    // Eliminar template
    async deleteMessageTemplate(id) {
        if (!this.pool) {
            logger_1.logger.info(`Mock template deleted: ${id}`);
            return true;
        }
        try {
            ...
            if (result.rows.length > 0) {
                logger_1.logger.info(`✅ Template eliminado: ${id}`);
                return true;
            }
            else {
                logger_1.logger.warn(`Template no encontrado: ${id}`);
            ...
            logger_1.logger.error('Error eliminando template:', error);
            return false;
        }
    }
    ...
    async incrementTemplateUsage(id) {
      ...
      `;
            ...
            logger_1.logger.error('Error incrementando uso de template:', error);
            return false;
        }
    }
    // ============================================
    ...
    async createProactiveMessage(data) {
        if (!this.pool) {
            logger_1.logger.info(`Mock proactive message created for lead: ${data.leadId}`);
      ...
      `;
            ...
            logger_1.logger.info(`✅ Mensaje proactivo creado para lead ${data.leadId}: ${messageId}`);
            return messageId;
        }
        catch (error) {
            logger_1.logger.error('Error creando mensaje proactivo:', error);
            return null;
        }
    }
    // Actualizar estado de mensaje proactivo
    async updateProactiveMessageStatus(id, status, errorMessage) {
        if (!this.pool) {
            logger_1.logger.info(`Mock proactive message status updated: ${id} -> ${status}`);
            return true;
        }
        try {
            let query = `
      ...
      `;
            ...
            const result = await this.pool.query(query, values);
            if (result.rows.length > 0) {
                logger_1.logger.info(`✅ Estado de mensaje proactivo actualizado: ${id} -> ${status}`);
                return true;
            }
            else {
                logger_1.logger.warn(`Mensaje proactivo no encontrado: ${id}`);
                return false;
            }
        }
        catch (error) {
            logger_1.logger.error('Error actualizando estado de mensaje proactivo:', error);
            return false;
        }
    }
    ...
    async getProactiveMessages(options = {}) {
      ...
      `;
            ...
            logger_1.logger.error('Error obteniendo mensajes proactivos:', error);
            return this.getMockProactiveMessages(options);
        }
    }
    ...
    getMockProactiveMessages(options) {
        const mockMessages = [
            {
                id: 'mock_proactive_1',
                leadId: '1',
        ...
        return mockMessages.filter(msg => {
            if (options.leadId && msg.leadId !== options.leadId)
                return false;
    ...
    async saveTrainingInteraction(interaction) {
        if (!this.pool) {
            logger_1.logger.warn('No database connection available, training interaction not saved');
            return null;
        }
        try {
            const query = `
      ...
      `;
            ...
            logger_1.logger.error('Error saving training interaction:', error);
            return null;
        }
    }
    // Obtener interacciones de entrenamiento
    async getTrainingInteractions(limit = 500) {
        if (!this.pool) {
            logger_1.logger.warn('No database connection available, returning empty training interactions');
            return [];
        }
        try {
            const query = `
      ...
      `;
            ...
            logger_1.logger.error('Error getting training interactions:', error);
            return [];
        }
    }
    // Añadir entrada a la knowledge base
    async addKnowledgeBase(entry) {
        if (!this.pool) {
            logger_1.logger.info(`Mock knowledge base entry added: ${entry.title}`);
            return true;
        }
        try {
      ...
      `;
            ...
            if (entryId) {
                logger_1.logger.info(`✅ Knowledge base entry added: "${entry.title}" (${entryId})`);
                return true;
            }
            return false;
        }
        catch (error) {
            logger_1.logger.error('Error adding knowledge base entry:', error);
            return false;
        }
    }
    ...
    async clearKnowledgeBase() {
        if (!this.pool) {
            logger_1.logger.info('Mock: Knowledge base cleared');
            return true;
        }
        try {
            const query = `DELETE FROM ai_knowledge_base`;
            const result = await this.pool.query(query);
            logger_1.logger.info(`🧹 Cleared ${result.rowCount} entries from knowledge base`);
            return true;
        }
        catch (error) {
            logger_1.logger.error('Error clearing knowledge base:', error);
            return false;
        }
    }
    ...
    async getKnowledgeBaseStats() {
      ...
      `;
            ...
            logger_1.logger.error('Error getting knowledge base stats:', error);
            return {
                totalEntries: 0,
                activeEntries: 0,
                categoryCounts: {},
    ...
    async updateKnowledgeBase(id, updates) {
        if (!this.pool) {
            logger_1.logger.info(`Mock knowledge base entry updated: ${id}`);
            return true;
        }
        try {
            const setParts = [];
      ...
      `;
            const result = await this.pool.query(query, values);
            if (result.rows.length > 0) {
                logger_1.logger.info(`✅ Knowledge base entry updated: ${id}`);
                return true;
            }
            else {
                logger_1.logger.warn(`Knowledge base entry not found: ${id}`);
                return false;
            }
        }
        catch (error) {
            logger_1.logger.error('Error updating knowledge base entry:', error);
            return false;
        }
    }
    ...
    async getTrainingStats() {
      ...
      `;
            ...
            logger_1.logger.error('Error getting training statistics:', error);
            return {
                totalInteractions: 0,
                averageSuccessScore: 0,
                interactionsLast7Days: 0,
    ...
    async searchTrainingInteractions(searchQuery, minSuccessScore, limit = 100) {
        if (!this.pool) {
            return [];
        }
        try {
      ...
      `;
            ...
            logger_1.logger.error('Error searching training interactions:', error);
            return [];
        }
    }
    // Eliminar interacciones de entrenamiento antiguas (limpieza de datos)
    async cleanupOldTrainingInteractions(daysOld = 90) {
        if (!this.pool) {
            return 0;
        }
        try {
      ...
      `;
            ...
            if (deletedCount > 0) {
                logger_1.logger.info(`🗑️ Cleaned up ${deletedCount} old training interactions (>${daysOld} days)`);
            }
            return deletedCount;
        }
        catch (error) {
            logger_1.logger.error('Error cleaning up old training interactions:', error);
            return 0;
        }
    }
}
