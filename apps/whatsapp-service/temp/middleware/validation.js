...
function validateCreateSession(req, res, next) {
    try {
        ...
        logger_1.logger.error('Error in validateCreateSession middleware:', error);
        res.status(500).json({
            success: false,
...
function validateSendMessage(req, res, next) {
    try {
        ...
        logger_1.logger.error('Error in validateSendMessage middleware:', error);
        res.status(500).json({
            success: false,
            error: 'Internal server error during validation',
            code: 'VALIDATION_ERROR'
...
function validateSessionId(req, res, next) {
    try {
        ...
        logger_1.logger.error('Error in validateSessionId middleware:', error);
        res.status(500).json({
            success: false,
            error: 'Internal server error during validation',
            code: 'VALIDATION_ERROR'
...
function rateLimit(req, res, next) {
    try {
        ...
        logger_1.logger.error('Error in rateLimit middleware:', error);
...
function logRequest(req, res, next) {
    ...
    // Log request start
    logger_1.logger.info(`<-- ${method} ${originalUrl}`, {
        ip: clientIp,
        userAgent,
