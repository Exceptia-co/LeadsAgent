...
async function testDatabase() {
  try {
    ...
    const DatabaseServiceModule = await import('./src/services/DatabaseService');
    ...
    // Test 5: Guardar respuesta IA
    console.log('');
    console.log('🤖 TEST 5: Guardando respuesta de IA...');
    const aiResponseData = {
      sessionId: testData.sessionId,
      phoneNumber: testData.phoneNumber,
      contactName: 'Usuario de Prueba',
      messageText: null,
      responseText: 'Esta es una respuesta automática de IA de prueba',
      messageType: 'text',
      intent: 'response',
      sentiment: 'helpful',
      aiProvider: 'gemini',
    ...
    const aiConversationId = await DatabaseService.saveConversation(aiResponseData);
    
    if (aiConversationId) {
      console.log('✅ Respuesta de IA guardada exitosamente!');
      console.log('🆔 ID de conversación:', aiConversationId);
    } else {
      console.log('❌ ERROR: No se pudo guardar la respuesta de IA');
    }
    
    // Test 6: Obtener contexto reciente
    console.log('');
    console.log('🔄 TEST 6: Obteniendo contexto reciente...');
    const context = await DatabaseService.getRecentContext(testData.phoneNumber, testData.sessionId, 5);
    
    console.log('📊 Contexto obtenido:', context.length, 'mensajes');
