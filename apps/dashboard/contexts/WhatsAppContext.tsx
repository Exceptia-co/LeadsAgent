...
export function WhatsAppProvider({ children }: { children: React.ReactNode }) {
  ...
  const createSession = useCallback(async (sessionData: any): Promise<boolean> => {
    try {
      ...
      showError('Error al crear sesión', err instanceof Error ? err.message : 'Error desconocido')
      return false
    }
  ...
  const deleteSession = useCallback(async (sessionId: string): Promise<boolean> => {
    try {
      const response = await fetch(`http://localhost:3002/sessions/${sessionId}`, {
        method: 'DELETE'
      })
      ...
      showError('Error al eliminar sesión', err instanceof Error ? err.message : 'Error desconocido')
      return false
    }
  }, [showError])

  const validateSessionForSending = useCallback(async (sessionId?: string): Promise<{ isValid: boolean; session?: WhatsAppSession; error?: string }> => {
    try {
      ...
      switch (targetSession.status) {
        ...
        default:
          return { 
            isValid: false, 
            session: targetSession,
            error: 'La sesión WhatsApp está desconectada. Reconecta la sesión e inténtalo nuevamente.' 
          }
      }
    } catch (err) {
