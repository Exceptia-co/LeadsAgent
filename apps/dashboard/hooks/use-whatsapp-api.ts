...
export function useWhatsAppApi() {
    ...
    async <T>(
      ...
      for (let attempt = 0; attempt <= retries; attempt++) {
        try {
          ...
          console.warn(
            `WhatsApp API attempt ${attempt + 1} failed, retrying in ${delayMs}ms:`,
            lastError.message,
          );
        }
      }
  ...
  // Analytics methods
  const getMessageAnalytics = useCallback(
    async (sessionId?: string, startDate?: string, endDate?: string) => {
