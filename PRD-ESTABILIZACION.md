...
> **STATUS: CLOSED 2026-04-18 — historical reference only.**
> Las 5 fases T0–T5 (incluyendo T0.4-ter) shipped en producción. Items aún
> marcados como `[ ]` en este doc (T0.3, T0.7, T2.1) fueron deferred-by-decision
> o resueltos sin actualizar este archivo. No editar este doc para nueva planificación;
> se mantiene como referencia auditiva. Para evolución continua del proyecto ver:
>
> - `PLAN-WHATSAPP-AGENT-MULTITENANT.md` — active plan (Fase B en curso).
> - `ANALISIS-ESTADO-PROYECTO.md` — snapshot de estado actualizado.
> - `docs/deployment/multi-tenant-rollout.md` — runbook + post-mortem de B.1.
...
|-----------|----------|--------------------------|
| **Frontend** | Vercel (Next.js) | Team `team_mP2bYgdUeXS5ArWzTHfw3RY5` · Project `dashboard` (`prj_3JGVC3KT0dnixeuZZwpcHTT0u3F6`), Node 24.x · dominio `guatsapp.me` · snapshot 2026-04-17; estado live ver `ANALISIS-ESTADO-PROYECTO.md` |
...
| **Firewall** | `whatsapp-firewall` (id 10443894) | Reglas `in`: 22/80/443/3002/3003 todos a `0.0.0.0/0` + `::/0` (estado snapshot 2026-04-17; cerrado en T0.2 — ver `ANALISIS-ESTADO-PROYECTO.md`). Sin reglas `out` |
| **Base de datos** | Supabase PostgreSQL `17.6.1.104` | Project `yxjzsargboxnuwnbuzax` (`CRMWhatsApp`), eu-west-3, `ACTIVE_HEALTHY` · **13 tablas en `public`** · **RLS OFF en 13/13** · **0 policies** en `pg_policies` · upgrade aplicado en T0.8; advisor `vulnerable_postgres_version` cerrado |
...
**Cambios requeridos:**
1. Instalar nginx/Caddy en el servidor como reverse proxy con TLS (certificados Let's Encrypt para `api.guatsapp.me`). Proxy pass:
...
- Servicios accesibles vía HTTPS a través del proxy en `api.guatsapp.me`.
...
- `vercel.json:44-46` → `/api/public/:path*` → `https://api.guatsapp.me/public/:path*`
...
En producción, si el puerto 3003 queda expuesto (ver T0.2) o si la URL `api.guatsapp.me/whatsapp/*` es accesible sin token, cualquiera puede enviar mensajes y manipular whitelist.
...
#### T0.4-ter — Añadir capa de auth al whatsapp-service Express ✅ cerrada (v5.23)
...
- Todo endpoint del whatsapp-service responde 401 sin credencial válida (verificado live: `curl -X POST :3002/api/proactive-messages` sin headers → `401 "missing x-service-timestamp or x-service-signature"`; con firma válida → handler llega normal).
- El router sólo responde en un punto de montaje único — eliminado `app.use('/', routes.default)`.
- Smoke tests pasaron (curl + jest unitarios).

**Decisión arquitectónica (MVP-fit):** opción (b) **HMAC shared secret**, no Clerk JWT. Razones: el whatsapp-service nunca atiende usuarios — todos sus clientes son backends (proxy Next + API Nest); Clerk JWT es user-scoped, forzarlo server-to-server requiere crear "service accounts" falsos (HMAC disfrazado); HMAC es ~1 ms vs 50-100 ms de JWT RSA verify + JWKS fetch; 0 deps nuevas.

**Fix aplicado:**
1. Nuevo middleware `apps/whatsapp-service/src/middleware/auth.ts` con `verifyServiceSignature` (HMAC-SHA256, ventana de 5 min anti-replay, `timingSafeEqual` para evitar timing attacks) + helper `signServiceRequest`. Bypass en `NODE_ENV=test` y en rutas `/health` / `/api/health` (liveness).
2. `apps/whatsapp-service/src/index.ts`: `express.json({ verify })` captura el `rawBody` buffer; `app.use(verifyServiceSignature)` montado antes de los routers; **eliminado el doble montaje** `app.use('/', routes)` — ahora sólo `/api`.
3. Helper duplicado (intencional, ~15 líneas cada uno, documentado) en `apps/dashboard/lib/service-auth.ts` y `apps/api/src/whatsapp/service-auth.ts`. Ambos generan el mismo formato: `headers = { 'x-service-timestamp', 'x-service-signature' }` donde `signature = sha256=${hex HMAC-SHA256(timestamp + '.' + rawBody, secret)}`.
4. Consumers actualizados:
   - `apps/dashboard/app/api/whatsapp/[...path]/route.ts`: lee el body una vez, firma, y lo reenvía al whatsapp-service con los 2 headers.
   - `apps/api/src/whatsapp/whatsapp.service.ts:sendMessage`: firma antes de hacer fetch.
5. Env var compartida: `WHATSAPP_SERVICE_HMAC_SECRET` (64 hex chars, generar con `node -e "require('crypto').randomBytes(32).toString('hex')"`). Añadido placeholder a `.env.example`; pendiente rotar e inyectar en Vercel + VPS Hetzner antes del próximo deploy.
6. Tests: `apps/whatsapp-service/src/middleware/auth.spec.ts` con 6 casos (firma válida → next; headers faltantes → 401; body tampered → 401; timestamp > 5 min → 401; `/api/health` sin firma → next; secret no configurado → 500). **6/6 PASS**.

**Acción operativa pendiente del PO antes del próximo deploy:**
- Añadir `WHATSAPP_SERVICE_HMAC_SECRET` (el mismo valor) en las env vars de Vercel (Production + Preview) y en `/srv/.env` del VPS Hetzner. Si no se configura, el servicio responderá 500 a todas las requests firmadas.
...
**Cambios requeridos:**
1. En Clerk Dashboard → Webhooks: crear endpoint `https://guatsapp.me/api/webhooks/clerk`.
...
- Código fuente visible públicamente incluyendo patrones de auth, schema Prisma, URLs de producción (`guatsapp.me`).
...
#### T5.1 — Tests para el flujo core de leads ✅ cerrada (v5.22)

**Fix aplicado:** Nuevo `apps/api/src/leads/leads.service.spec.ts` con 10 tests unitarios (mocks de `PrismaService`): `create` (happy path + duplicate phone), `findAll` (filtro `deletedAt: null` + combinación de q/status), `findOne` (lead existente + NotFound al estar soft-deleted), `remove` (soft delete via update, no hard delete), `getStats` (cada counter incluye `deletedAt: null`), `updateStatus`, `updateWhatsAppAuth`. Los tests validan explícitamente las semánticas de T4.1.

#### T5.2 — Tests para el flujo de WhatsApp webhook ✅ cerrada parcial (v5.22)

**Fix aplicado:** Nuevo `apps/api/src/whatsapp/whatsapp.controller.spec.ts` con 6 tests del controller focalizados en el boundary: header `x-whatsapp-service` obligatorio (`BadRequestException` sin él), y la tabla de dispatch para los 4 eventos (`message`, `authenticated`, `disconnected`, `status_change`) más el no-op de `qr_updated`. Usa `overrideGuard(ClerkAuthGuard).useValue({ canActivate: () => true })`.

**Follow-up:** el HMAC real entre Nest↔whatsapp-service queda con T0.4-ter, así que la verificación criptográfica del webhook se añadirá cuando esa tarea se cierre.
...
| v5.23 | 2026-04-18 | **T0.4-ter cerrada (HMAC service-to-service).** Decisión: opción (b) HMAC shared secret sobre opción (a) Clerk JWT — el whatsapp-service sólo tiene clientes backend (proxy Next + API Nest), HMAC es idiomático server-to-server, cero deps nuevas, ~1 ms/request vs 50-100 ms de JWT. Nuevo middleware `verifyServiceSignature` + helper `signServiceRequest` en `apps/whatsapp-service/src/middleware/auth.ts`. Bootstrap reemplazado: `express.json({ verify })` captura `rawBody`; el middleware monta antes de los routers; **doble montaje eliminado** (`app.use('/', routes)` borrado, sólo queda `/api`). Helper duplicado 1:1 en `apps/dashboard/lib/service-auth.ts` y `apps/api/src/whatsapp/service-auth.ts`. Dashboard proxy `/api/whatsapp/[...path]` y `WhatsAppService.sendMessage` del API Nest ahora firman cada request con `x-service-timestamp` + `x-service-signature` (sha256 HMAC de `timestamp + "." + body`). Validación: curl sin firma → 401 `missing x-service-timestamp or x-service-signature`; curl con firma válida → handler normal. Tests jest unitarios: **6/6 PASS** (happy path, headers faltantes, body tampered, timestamp > 5 min, health whitelist, secret no configurado). Env var nueva `WHATSAPP_SERVICE_HMAC_SECRET` en `.env.example`. Acción PO pendiente: rotar e inyectar el secret en Vercel y en VPS Hetzner antes del próximo deploy. |
| v5.22 | 2026-04-18 | **T5.1 + T5.2 cerradas (tests unitarios Leads + WhatsApp webhook).** `apps/api/src/leads/leads.service.spec.ts`: 10 tests con mocks de `PrismaService` cubren los 7 métodos del service + la semántica soft-delete de T4.1 (filtros `deletedAt: null`, `remove` como update). `apps/api/src/whatsapp/whatsapp.controller.spec.ts`: 6 tests del boundary (header `x-whatsapp-service` obligatorio, dispatch table para 4 eventos + noop de `qr_updated`) con `overrideGuard(ClerkAuthGuard).useValue({ canActivate: () => true })`. `pnpm jest` local: **16/16 PASS** en 4.53 s. Follow-up T0.4-ter añadirá la verificación HMAC del webhook. |
