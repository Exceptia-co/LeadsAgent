...
4. **Paso 4 — DROP columnas duplicadas (destructivo, pendiente autorización PO):** `ALTER TABLE whatsapp_conversations DROP COLUMN message_text, DROP COLUMN response_text`. Los readers ya caen al FK en su ausencia; el único riesgo es pérdida de `message_type` adicional en `message.messageType` (que ya existe). Requiere autorización explícita del PO.
5. **Paso 5 — DROP tabla legacy `migrations` (destructivo, pendiente autorización PO):** 2 filas de hotfixes de septiembre 2025, no referenciadas por el código. Bloqueado en Phase A por safety policy.
