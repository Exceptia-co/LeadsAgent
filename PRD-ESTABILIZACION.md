...
2. **Paso 2 — Backfill de huérfanos históricos (pendiente):** matchear los 58 huérfanos contra `messages` por `(phone_number, message_text, created_at ±30s)`. Los que no matcheen quedan `message_id = NULL` (aceptable).
3. **Paso 3 — Readers (pendiente):** migrar `ai-thinking/ContextEnricher.ts:90` y `ai-thinking/analysis/ContextEnricher.ts:88` + sus tests a JOIN Prisma (`include: { message: true }`) — hoy leen `message_text`/`response_text` directos.
4. **Paso 4 — DROP columnas duplicadas (destructivo):** `ALTER TABLE whatsapp_conversations DROP COLUMN message_text, DROP COLUMN response_text`. Requiere que los readers ya usen JOIN.
5. **Paso 5 — DROP tabla legacy `migrations` (destructivo):** bloqueado en Phase A por safety policy — requiere autorización explícita del PO.
