...
// Ejemplo de uso
classifyLead(
  "Hola, me interesa comprar su producto urgentemente",
  "+34123456789",
  ...
  for (let i = 0; i < maxAttempts; i++) {
    try {
      const response = await fetch(
        `http://localhost:3002/sessions/${sessionId}/qr`,
      );
      const result = await response.json();
...
async function sendWhatsAppMessage(
  ...
  try {
    const response = await fetch(
      `http://localhost:3002/sessions/${sessionId}/send`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
async function getWhitelistLogs(filters = {}) {
  try {
    ...
    const response = await fetch(
      `http://localhost:3002/logs/whitelist?${queryParams}`,
    );
    ...
    if (logs.success) {
      ...
      logs.data.forEach((log) => {
        console.log(
          `${log.timestamp}: ${log.phoneNumber} - ${log.decision} (${log.reason})`,
        );
      });
    }
...
const message =
  "Hola, necesito comprar urgentemente su producto para mi empresa. ¿Cuánto cuesta y cuándo pueden entregarlo? Mi email es cliente@empresa.com";
...
async function testProvider(provider: "openrouter" | "gemini") {
  try {
    ...
    if (result.success) {
      ...
      console.log(
        `💡 Check ${provider.toUpperCase()}_API_KEY in environment variables`,
      );
    }
  } catch (error) {
...
async function troubleshootWhatsAppSession(sessionId: string) {
  ...
  try {
    ...
    const statusResponse = await fetch(
      `http://localhost:3002/sessions/${sessionId}/status`,
    );
    const status = await statusResponse.json();
    ...
    switch (status.data?.status) {
      case "connecting":
        ...
        console.log(
          "💡 Solution: Wait for QR code generation or check for QR code",
        );
...
async function recreateSession(sessionId: string) {
  ...
  try {
    await fetch(`http://localhost:3002/sessions/${sessionId}`, {
      method: "DELETE",
    });
...
class PerformanceMonitor {
  ...
  public getStats(operation: string) {
    ...
    const p95 = metrics.sort((a, b) => a - b)[
      Math.floor(metrics.length * 0.95)
    ];
