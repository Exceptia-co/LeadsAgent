...
- `destroySession(sessionId, mode: 'shutdown' | 'delete')` — **`'shutdown'` must not clear credentials.** This is the Phase 1 Task 1 contract; breaking it reintroduces the bug that logged every session out on every deploy.
  ...
  In `'delete'` mode the order is **unsubscribe first, wipe second**: add to `stopped` → `ev.off` the three listeners → `sock.logout()` → `sock.end()` → `store.clear(sessionId)`. A `creds.update` already in flight when the wipe runs would otherwise land *after* it and write fresh rows into `whatsapp_auth_keys` for a session that was just deleted — leaving orphaned credentials that no longer match any live socket, and that `hasCredentials` will happily report as a session ready to recover.
