# EscortsHub - Información de Productos y FAQ's

## Productos Disponibles

### 1. ANUNCIO DOBLE

- **Descripción**: Anuncio con visibilidad aumentada que ocupa doble espacio
- **Beneficios**: Mayor visibilidad en los listados
- **Ideal para**: Escorts que buscan destacar entre los anuncios regulares

### 2. ANUNCIO TOP

- **Descripción**: Anuncio destacado en posición superior
- **Beneficios**: Posicionamiento privilegiado en la parte superior de los listados
- **Ideal para**: Escorts que buscan máxima exposición

### 3. ANUNCIO DOBLE TOP

- **Descripción**: Combinación de anuncio doble y posición top
- **Beneficios**: Máxima visibilidad y espacio destacado
- **Ideal para**: Escorts premium que buscan dominar los listados

### 4. DISPONIBLE AHORA

- **Descripción**: Indicador de disponibilidad inmediata
- **Beneficios**: Muestra a los clientes que estás disponible en tiempo real
- **Ideal para**: Escorts con disponibilidad inmediata

### 5. HISTORIAS

- **Descripción**: Función para compartir contenido temporal
- **Beneficios**: Permite mostrar actualizaciones y contenido dinámico
- **Ideal para**: Mantener el perfil actualizado y mostrar novedades

### 6. REACTIVACIÓN

- **Descripción**: Servicio para reactivar anuncios pausados
- **Beneficios**: Permite volver a activar anuncios anteriores
- **Ideal para**: Escorts que retoman su actividad

## Sistema de Precios y Moneda HUB

### Moneda HUB

- **¿Qué es?**: Es la moneda virtual de la plataforma utilizada para activar anuncios
- **Uso**: Se intercambia por días de activación de anuncios
- **Ventajas**: Sistema flexible que permite gestionar múltiples anuncios

### Precios de los Productos (en Monedas HUB)

1. **ANUNCIO DOBLE** (11 monedas HUB base)
   - 1 día: 20 monedas HUB
   - 5 días: 85 monedas HUB
   - 10 días: 150 monedas HUB
   - Beneficio: Mayor visibilidad con espacio doble

2. **ANUNCIO TOP** (28 monedas HUB base)
   - 3 días: 85 monedas HUB
   - 7 días: 125 monedas HUB
   - 10 días: 165 monedas HUB
   - 30 días: 450 monedas HUB
   - Beneficio: Posicionamiento superior garantizado

3. **ANUNCIO DOBLE TOP** (59 monedas HUB base)
   - 3 días: 170 monedas HUB
   - 7 días: 250 monedas HUB
   - 10 días: 330 monedas HUB
   - 30 días: 900 monedas HUB
   - Beneficio: Máxima visibilidad combinada

4. **DISPONIBLE AHORA** (100 monedas HUB base)
   - 10 unidades: 40 monedas HUB
   - 25 unidades: 100 monedas HUB
   - 100 unidades: 400 monedas HUB
   - Beneficio: Indicador de disponibilidad inmediata

5. **HISTORIAS** (seven monedas HUB base)
   - 1 unidad: 12 monedas HUB
   - 5 unidades: 60 monedas HUB
   - 10 unidades: 110 monedas HUB
   - Beneficio: Contenido temporal destacado

6. **REACTIVACIÓN** (10 monedas HUB base)
   - 1 unidad: 25 monedas HUB
   - 5 unidades: 115 monedas HUB
   - 10 unidades: 215 monedas HUB
   - Beneficio: Recuperación rápida de anuncios pausados

### Compra de Monedas HUB

- **Tasa de cambio**: Las monedas HUB tienen un valor variable según el paquete seleccionado
- **Paquetes disponibles**:
  - Paquete Básico: 100 monedas HUB por 80,00 EUR (0,80 € por moneda)
  - Paquete Estándar: 200 monedas HUB por 150,00 EUR (0,75 € por moneda)
  - Paquete Plus: 500 monedas HUB por 300,00 EUR (0,60 € por moneda)
  - Paquete Premium: 1.000 monedas HUB por 700,00 EUR (0,70 € por moneda)

### Beneficios por Paquete

- **Paquete Básico**: Ideal para probar el servicio o anuncios cortos
- **Paquete Estándar**: Mejor relación calidad-precio para uso regular
- **Paquete Plus**: Máximo ahorro por moneda (0,60€/moneda)
- **Paquete Premium**: Gran cantidad de monedas con buen descuento

### Consejos de Compra

- Los paquetes de más días/unidades ofrecen mejor relación precio-beneficio
- El Anuncio Doble Top ofrece la máxima visibilidad combinando las ventajas del anuncio doble y top
- Las Historias son ideales para mantener el perfil actualizado y dinámico
- El servicio "Disponible Ahora" es perfecto para gestionar la disponibilidad en tiempo real

### Métodos de Pago

- Tarjetas de crédito/débito
- Transferencia bancaria
- Métodos de pago digitales
- Criptomonedas (consultar disponibilidad)

### Wallet (Monedero)

- **Acceso**: Disponible en el dashboard del usuario
- **Funciones**:
  - Consulta de saldo
  - Historial de transacciones
  - Recarga de monedas HUB
  - Gestión de gastos

## Preguntas Frecuentes (FAQ's)

### Generales

1. **¿Qué es EscortsHub?**
   - Es el sitio web original de escorts en España, una plataforma para anuncios de servicios de acompañantes.

2. **¿Es necesario ser mayor de edad?**
   - Sí, es obligatorio ser mayor de 18 años para acceder y utilizar la plataforma.

### Sobre los Anuncios

1. **¿Qué tipo de anuncios puedo publicar?**
   - Hay varios formatos disponibles: normal, doble, top, y doble top.

2. **¿Cuál es la diferencia entre los tipos de anuncios?**
   - Anuncio Normal: Formato estándar
   - Anuncio Doble: Mayor espacio y visibilidad
   - Anuncio Top: Posición destacada superior
   - Anuncio Doble Top: Combina espacio doble y posición superior

### Funcionalidades

1. **¿Qué es "Disponible Ahora"?**
   - Es una función que permite mostrar tu disponibilidad en tiempo real a los clientes.

2. **¿Cómo funcionan las Historias?**
   - Son publicaciones temporales que permiten compartir actualizaciones y contenido dinámico.

### Privacidad y Seguridad

1. **¿Qué políticas de privacidad aplican?**
   - El sitio utiliza cookies propias y de terceros para mejorar servicios y analizar tráfico.
   - Existe una política de cookies detallada disponible para consulta.

2. **¿Cómo se protege la privacidad de los usuarios?**
   - Se requiere confirmación de edad
   - Acceso voluntario y consciente
   - Políticas de cookies transparentes

### Gestión de Cuenta

1. **¿Qué es la Reactivación?**
   - Es un servicio que permite volver a activar anuncios que han sido pausados.

2. **¿Puedo pausar y reactivar mis anuncios?**
   - Sí, existe la opción de reactivación para anuncios pausados.

## Notas Importantes

- El sitio utiliza cookies para mejorar la experiencia del usuario
- Se requiere aceptación de la política de cookies para navegar
- Toda la información está sujeta a la política de privacidad del sitio
