...
- [ ] **B1.8-bis.** Crear entorno local/staging migrable para smoke de PR1. El smoke runtime de esta rama no debe usar Supabase producción sin aplicar migration: Prisma Client generado selecciona columnas nuevas (`tenant_id`, etc.) aunque el código no las lea explícitamente, y endpoints como `/leads`/`/templates` devuelven 500 si la DB sigue vieja.
