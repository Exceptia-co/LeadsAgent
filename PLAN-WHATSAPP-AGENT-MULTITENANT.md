...
- [ ] **B1.2(a).** **Resolución de `tenant_id` para `TenantContextGuard` (PR4)** — bloqueado por plan Clerk Student. El middleware de PR3 NO depende de esto (usa `orgId` built-in). Solo lo necesita el guard de NestJS en PR4. **Decisión a tomar entre 2 vías mutuamente excluyentes**, no las dos:
  ...
  - **Vía recomendada (default)**: B en PR4 hasta que el negocio justifique el upgrade. La Vía A se reactiva si y solo si se requiere `getToken({template:'supabase'})` (B1.2(b)) para RLS directa, donde el claim sí es prerequisito técnico inexcusable.
