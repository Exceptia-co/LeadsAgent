...
- **Última revisión:** 2026-04-20 (v7.3 — Fase A desplegada en prod Hetzner + B1.15 añadida por deuda T4)
...
- **Estado:** **Fase A ejecutada + desplegada en prod Hetzner** (2026-04-20) — PR #10 mergeado a `develop` (commit `1fd9e95`). Próximo paso: **Fase B.1 empezando por B1.15** (NestJS `ConfigModule`, bloquea B1.2(a)).
...
- Doble init del whatsapp-service (log sugiere, handler dispara 1 sola vez por evento — investigar Fase C)
...
- [ ] **B1.3.** Migración Prisma: crear tabla `tenants` (id UUID PK, clerk_org_id VARCHAR UNIQUE, name, plan, settings)
- [ ] **B1.4.** Migración Prisma: añadir `tenant_id UUID FK` a **las 12 tablas de datos** que llevan `tenant_id` — la tabla `tenants` no lleva `tenant_id` porque es la raíz. Tablas: `whatsapp_sessions`, `leads`, `messages`, `whatsapp_conversations`, `proactive_messages`, `message_templates` [v6: pasa a tenant-scoped], `whatsapp_whitelist_logs`, `ai_training_interactions`, `ai_knowledge_items` [rename de `ai_knowledge_base`], `ai_agents` [nueva], `ai_products` [nueva], `ai_configuration` [nullable]. Total del ámbito multi-tenant: **1 raíz + 11 tablas con policy normal + 1 tabla mixta (`ai_configuration`) = 13 tablas con RLS activa**
- [ ] **B1.5.** Migración Prisma: cambiar `Lead.phone` de `@unique` global → `@@unique([tenantId, phone])`
- [ ] **B1.6.** Migración Prisma: convertir `WhatsAppConversation.sessionId` (VARCHAR suelto) → FK real `whatsappSessionId UUID` con `@relation` a `WhatsAppSession`. Mantener `sessionId` string como columna legacy deprecada 1 versión
- [ ] **B1.7.** Migración Prisma: crear `ai_agents` (N-por-tenant, campos §4.1) + `ai_products` + renombrar `ai_knowledge_base` → `ai_knowledge_items` con FKs `tenant_id` y `ai_agent_id`
- [ ] **B1.8.** **Índices B-tree** en cada nueva columna `tenant_id` (obligatorio para performance)
...
- [ ] **B1.10.** Webhook `POST /api/webhooks/clerk/organizations` (NestJS, ruta en plural) sincroniza eventos `organization.created|updated|deleted` → CRUD local de `Tenant`. **Tras crear `Tenant`, hace `PATCH https://api.clerk.com/v1/organizations/{org_id}/metadata` con body `{ "public_metadata": { "tenant_id": "<uuid>" } }`** usando `CLERK_SECRET_KEY` (v6.5: ruta corregida al endpoint dedicado `/metadata`). Esto garantiza que el claim `tenant_id` estará disponible en session tokens posteriores. Verificación Svix con `CLERK_ORG_WEBHOOK_SECRET` dedicado. **Importante (v6.5)**: la verificación Svix debe usar `req.rawBody` capturado en `apps/api/src/main.ts:9-23` (que ya existe en el repo para el HMAC del whatsapp-service), **no el body parseado por Nest/Express**; de lo contrario la firma falla por diferencia de bytes
...
- [ ] **B1.15.** **NestJS api lee `.env` via `ConfigModule`** (añadido v7.3 tras deuda T4 detectada 2026-04-20 en prod Hetzner). Hoy `leadcrm-api` solo recibe envs desde la shell que ejecutó PM2, no desde `apps/api/.env`. Fix: en `apps/api/src/app.module.ts` importar `ConfigModule.forRoot({ isGlobal: true, envFilePath: ['.env', '../../.env'] })` antes de los demás imports. Verificación: tras restart de `leadcrm-api`, `WHATSAPP_ALLOW_NEW_LEADS` debe aparecer en `/proc/$PID/environ` sin `export` previo al `pm2 restart`. **Bloquea B1.2(a)** porque Clerk session token config + webhook secrets leerán envs que hoy no se cargan en la api. Estimación: Quick (<30 min).
...
| Backfill de `tenant_id` asigna filas al tenant incorrecto                                              | Baja             | Alto        | Dry-run en branch Supabase; verificación manual de las 24 sesiones existentes antes de aplicar en prod                                                                         |
| Lead con mismo `phone` en dos tenants colisiona por unicidad global vieja                              | Alta (al migrar) | Alto        | B1.5 cambia unicidad a compuesta ANTES del backfill. Verificar constraint antes de asignar tenants                                                                             |
...
| 9   | ¿`WhatsAppConversation.sessionId` string o FK?               | **FK real** (`whatsappSessionId UUID`). Mantener string 1 versión como legacy y luego borrar                                |
| 10  | ¿`Lead.phone` unicidad?                                      | **`@@unique([tenantId, phone])`** (no global). Dos tenants pueden tener el mismo teléfono                                   |
...
- `packages/db/prisma/schema.prisma:134-148` — `ai_knowledge_base` a renombrar y extender
- `packages/db/prisma/schema.prisma:239-262` — `WhatsAppSession` a extender con FKs
