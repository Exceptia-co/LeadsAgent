...
- **Cliente WhatsApp:** `whatsapp-web.js` es el motor **actual**. El 2026-08-25 se acordó
  **migración directa a `@whiskeysockets/baileys`** — motor único, borrando wwebjs, sin
  adapter dual ni flag `WA_ENGINE`. **Aún no ejecutada.** Motivo: el techo de ~5–8 sesiones
  por VPS (§2.1) es de Chromium, no del producto, y la fase piloto es la ventana barata para
  cambiarlo. Sigue en pie que **no** se migra a WhatsApp Business API en este plan (ver R1).
...
2. **Configurar Clerk en dos lugares distintos (v6.3 — antes confundía estos conceptos)**:
   - **(a) Customize session token** (`Dashboard Clerk → Sessions → Customize session token`): añadir al JSON el claim `"tenant_id": "{{org.public_metadata.tenant_id}}"`. **Importante (post-PR4 review)**: este claim **NO** es necesario para el middleware Next.js — `auth()` ya devuelve `orgId` built-in y el middleware decide redirects con `orgId` solo. El claim `tenant_id` sólo se necesita si se elige la **Vía A** de B1.2(a) para `TenantContextGuard` (lectura directa sin DB hit). La Vía B (lookup server-side por `orgId` contra tabla `tenants`) es el default recomendado y NO requiere este claim. **Solo es prerequisito inexcusable para `getToken({template:'supabase'})` en B1.2(b)** (RLS directa). En plan Student de Clerk el save de custom claims está bloqueado, lo que refuerza la elección de Vía B por defecto.
...
9. **Row Level Security (RLS) como defensa secundaria** — **policies completas con USING + WITH CHECK (v6.2)**:
   ...
   - **Mecanismo concreto para `tenant_id` en JWT** (antes estaba subespecificado):
     ...
     - **Customize session token** en Clerk Dashboard añade el claim: `"tenant_id": "{{org.public_metadata.tenant_id}}"`. **Aclaración post-PR4**: este claim **no** es leído por el middleware Next.js de `apps/dashboard` — el middleware solo decide redirects con `orgId` (built-in). Sólo el `TenantContextGuard` de NestJS (PR5) lo lee, **y únicamente si se opta por Vía A de B1.2(a)**. La Vía B (default recomendada) ignora este claim y resuelve `tenantId` por lookup contra la tabla `tenants`.
     - **JWT Template "supabase"** (opcional, solo si se usa Supabase client con JWT): mismo claim `tenant_id`. Así RLS puede leer `auth.jwt() ->> 'tenant_id'` cuando el dashboard haga queries `anon` firmadas con Clerk. **Único caso donde el custom claim es prerequisito técnico inexcusable** (no opcional).
     - **Resolución `tenantId` desde `orgId` (Vía B, default recomendada en PR5)**: el `TenantContextGuard` recibe `orgId` (built-in en session token), hace lookup `prisma.tenant.findUnique({ where: { clerkOrgId: orgId } })` (con cache LRU 5 min en memoria + opcional Redis). Sin coste de upgrade Clerk, latencia despreciable. Esta vía también cubre la "race condition token-emitido-antes-de-sync" — si el `Tenant` local no existe todavía (webhook aún no procesado), el guard puede crearlo on-the-fly o devolver 404 con retry.
     - **Vía A — Custom session claim** (alternativa, requiere upgrade plan Clerk): tras configurar B1.2(a), el guard lee `auth().sessionClaims.tenant_id` directo sin DB hit. Tras webhook que actualiza `publicMetadata`, el frontend fuerza refresh con **`await getToken({ skipCache: true })`** o **`await user.reload()`** para que el JWT incluya el claim actualizado.
...
- [x] **B1.1.** Clerk Organizations activado en **Clerk Development** (instance `ins_31WLEulvioak3keE58HPWDIQ2Gu`) 2026-05-01. Pendiente activarlo en **Clerk Production** (instance `ins_38q4uuEAb2DHHjvZIwa6r3rEare`) antes del deploy de Fase B.1 a prod (Vercel/Hetzner).
- [x] **B1.1a.** **Organización "EscortsHub"** creada en **Clerk Development** 2026-05-01: `org_3D8ZN8vTbv1rUvD22QWRVkzjO2J`. Sin owner asignado todavía (la Org se creó vía Dashboard sin `created_by`; el admin actual debe ser invited via "Members → Add member" para poder seleccionarla en `/select-org`). Pendiente crear la org equivalente en **Clerk Production** antes del deploy.
- [x] **B1.1b.** **UI de selección de organización**: implementado en `feature/b1-auth-shell`. `apps/dashboard/app/select-org/page.tsx` (Server Component con `auth()` guards: sin `userId` → `/sign-in`, con `orgId` → `/dashboard`, sin org → renderiza `<OrgPicker />`) + `org-picker.tsx` Client Component aislado con `<OrganizationList hidePersonal afterSelect/afterCreate=/dashboard>`. `<OrganizationSwitcher>` añadido en las **3 ubicaciones** con `<UserButton>` del `dashboard/layout.tsx` (mobile sidebar footer + desktop sidebar footer + mobile header) — footers como fila separada con `space-y-3`, mobile header en contenedor compacto `max-w-[180px]` para evitar overflow. Smoke local 2026-05-02 verde.
- [ ] **B1.2(a).** **Resolución de `tenant_id` para `TenantContextGuard` (PR5a runtime)** — bloqueado por plan Clerk Student. El middleware Next.js (PR3) NO depende de esto (usa `orgId` built-in). Solo lo necesita el guard de NestJS en **PR5a**. **Decisión a tomar entre 2 vías mutuamente excluyentes**, no las dos:
  - **Vía A — Custom session token claim**: en Clerk Dashboard → Sessions → Customize session token añadir `"tenant_id": "{{org.public_metadata.tenant_id}}"`. Bloqueado actualmente: el plan Student rechaza el save con "Could not save settings" (verificado con 3 variantes de JSON 2026-05-02). Requiere upgrade a plan Pro o superior. Una vez activo, el guard lee `auth().sessionClaims.tenant_id` directo sin DB hit.
  - **Vía B — Lookup server-side via Clerk Backend SDK**: el guard recibe `orgId` (built-in en session token, ya disponible sin upgrade), llama `clerkClient.organizations.getOrganization({organizationId: orgId})` o consulta tabla local `tenants WHERE clerk_org_id = orgId` para obtener `tenantId`. Sin coste de upgrade pero añade 1 round-trip por request (mitigable con cache LRU 5min en memoria + Redis). Compatible con plan Student.
  - **Vía recomendada (default)**: B en PR5a hasta que el negocio justifique el upgrade. La Vía A se reactiva si y solo si se requiere `getToken({template:'supabase'})` (B1.2(b)) para RLS directa, donde el claim sí es prerequisito técnico inexcusable.
...
- [ ] **B1.5.** Migración Prisma: cambiar `Lead.phone` de `@unique` global → `@@unique([tenantId, phone])`. **PR5b destructive cleanup, post-backfill prod**: quitar el unique global antes de poblar `tenant_id` permitiría duplicados porque Postgres permite múltiples `NULL` en unique compuesto. Solo seguro tras verificar `tenant_id IS NULL` count = 0 en `leads` prod.
...
- [x] **B1.6(b) — backfill parcial**. PR4 commit `42bb7a9` añade `whatsapp_session_id` via JOIN contra `whatsapp_sessions.session_id` en el script `backfill-tenant.ts`. Reporta huérfanos (filas sin matching `session_id`) sin error. **Pendientes** (PR5): actualizar consumers para que lean `whatsappSessionId` en lugar de `sessionId`, gestionar huérfanos restantes (mantener / archivar / force-NULL), DROP de `session_id` VARCHAR en PR5b destructive cleanup tras verificar 0 huérfanos en prod.
...
- [ ] **B1.7(b).** **PR5b destructive cleanup**: renombrar `ai_knowledge_base` → `ai_knowledge_items` en la misma iteración que actualice SQL raw en `apps/whatsapp-service/src/services/DatabaseService.ts:176` y `packages/db/prisma/seed.ts:164`. Migration con `ALTER TABLE ... RENAME TO` (no DROP+CREATE — destructivo). Solo seguro tras `agent_id` ya backfilled (hecho en PR4 commit `42bb7a9`) y consumers actualizados.
...
- [x] **B1.9 (script)**. **Backfill script** implementado en `feature/b1-backfill` (`packages/db/scripts/backfill-tenant.ts`). Diseño agnóstico de instancia Clerk: `--clerk-org-id <id>` como CLI arg (o env `CLERK_ORG_ID`); `--dry-run` por defecto; `--apply` requiere `--confirm I-UNDERSTAND-THIS-MUTATES` para mutar (gate operacional anti-accidente); idempotente (re-ejecutar es no-op). Steps: (1) find/create `Tenant` por `clerk_org_id`, (2) `UPDATE tenant_id WHERE NULL` en 9 tablas tenant-scoped — `ai_configuration` se excluye intencionalmente porque su `tenant_id NULL` es semántico (configs globales); (3) crear `AiAgent` default `<TenantName> Default` si no existe (defaults Prisma: tone=FRIENDLY, primaryGoal=CONTACT, language=es); (4) `--patch-clerk` opcional para PATCH metadata (default false: el webhook `organization.updated` lo hará al re-emitir). Smoke local 2026-05-02 verde: dry-run muestra 27 rows en 9 tablas, apply muta exitosamente, re-apply 0 rows (idempotente), `--apply` sin `--confirm` rechazado con exit 2. **Asociación a `whatsapp_sessions.aiAgentId`** queda para PR5 cuando el agent se use realmente. **NO toca constraints `NOT NULL` ni `@@unique([tenantId, phone])`** — eso es PR5 enforcement.

  **Pendientes de B1.9 (runbooks operativos separados, no parte de este PR)**:
  - **(staging)** Crear branch Supabase, aplicar migraciones de PR1, ejecutar dry-run + apply contra esa branch para validar el flow contra una copia prod-like
  - **(prod)** Crear org "EscortsHub" en Clerk Production (instance `ins_38q4uuEAb2DHHjvZIwa6r3rEare`) y obtener su `org_xxx` Production. Aplicar migraciones PR1 a Supabase prod. Ejecutar backfill con el `org_xxx` Production. PATCH metadata Clerk Production via `--patch-clerk`. Smoke runtime end-to-end

- [x] **B1.10.** Webhook `POST /api/webhooks/clerk/organizations` (NestJS, ruta en plural) sincroniza eventos `organization.created|updated|deleted` → CRUD local de `Tenant`. **Tras crear `Tenant`, hace `PATCH https://api.clerk.com/v1/organizations/{org_id}/metadata` con body `{ "public_metadata": { "tenant_id": "<uuid>" } }`** usando `CLERK_SECRET_KEY` (v6.5: ruta corregida al endpoint dedicado `/metadata`). Esto garantiza que el claim `tenant_id` estará disponible en session tokens posteriores. Verificación Svix con `CLERK_ORG_WEBHOOK_SECRET` dedicado. **Importante (v6.5)**: la verificación Svix debe usar `req.rawBody` capturado en `apps/api/src/main.ts:9-23`, **no el body parseado por Nest/Express**. Implementación PR2 en `feature/b1-clerk-organizations` (commits `d7be454`, `5a6d171`): módulo Nest + tests unitarios verdes; smoke local firmado Svix verde; **smoke real verde 2026-05-01** (Clerk Development): org "EscortsHub" creada en Clerk Development (`org_3D8ZN8vTbv1rUvD22QWRVkzjO2J`), webhook firmado→201 `action=synced`, `Tenant` local creado (`fc732fd0-095b-4be7-b1ee-eb9b0b37580c`), Clerk API confirma `publicMetadata.tenant_id=fc732fd0-...` post-PATCH, firma inválida rechazada con 400 sin mutar DB. **PRODUCCIÓN cerrada 2026-05-03 19:51 UTC**: webhook configurado en Clerk Production instance `ins_38q4uuEAb2DHHjvZIwa6r3rEare` apuntando a `https://api.guatsapp.me/api/webhooks/clerk/organizations`, suscrito a los 3 eventos `organization.*`, `CLERK_ORG_WEBHOOK_SECRET` sincronizado en Hetzner `/opt/leadcrm/apps/api/.env`. Smoke real verde end-to-end con org `WebhookSmokeTest`: create → fila tenants ↔ patch metadata → delete → fila desaparece, todo en ~2 minutos sin FK errors (test org sin relaciones). Comportamiento sobre orgs con FK references documentado como deuda en `docs/deployment/multi-tenant-rollout.md` gap 3.5.
- [x] **B1.11.** Middleware Next.js implementado en `feature/b1-auth-shell`. Patrón final acotado a `/` y `/dashboard(.*)` para preservar el contrato JSON 401 de las API routes privadas (Codex review crítico — no usar `!isPublicRoute` como guard general). 3 casos: (A) `isProtectedRoute` sin `userId` → `/sign-in`; (B) `isProtectedRoute` con `userId` sin `orgId` → `/select-org`; (C) ruta `/` con `userId` → `/dashboard` si hay `orgId`, `/select-org` si no. `/select-org(.*)` añadido a `isPublicRoute` y delegado a la página Server (defensa en profundidad). Smoke runtime 2026-05-02 verde: `curl -i /api/whatsapp/sessions` sin auth → `401 application/json` (no 307 HTML); `/dashboard` sin sesión → `/sign-in`; `/dashboard` con sesión sin org → `/select-org` (verificado en browser); `/` con sesión sin org → `/select-org` (verificado en browser); `/select-org` sin sesión → `/sign-in` (server-side guard). **B1.2(a) deferido**: el plan Student de Clerk rechaza guardar custom session token claims (verificado con 3 variantes de JSON, todas fallan con "Could not save settings"). El middleware actual no necesita `tenant_id` (solo `orgId` built-in); B1.2(a) se reactiva en **PR5a** cuando `TenantContextGuard` lo necesite, eligiendo Vía B (lookup server-side por orgId) como default — compatible con plan Student.

  Cambios concretos del middleware original (v6.2):
...
- [x] **B1.12.** Guard NestJS `TenantContextGuard`: extrae `orgId`, carga `Tenant`, inyecta `tenantId` en request. Decorator `@CurrentTenant()`. **Shipped en PR #11 commit `dcf81dd`**: implementación en `apps/api/src/auth/tenant-context.guard.ts` con tests `tenant-context.guard.spec.ts`. Guard activo en cada controller protegido del API Nest (12 controllers) + middleware equivalente en whatsapp-service (`apps/whatsapp-service/src/middleware/tenant-guard.ts`). Cross-tenant requests devuelven 404 (no 403 — evitar leak de id-existence). Cache LRU 60s en memoria reduce DB hits a ~1 por 60s por tenant.
- [ ] **B1.13.** **Prisma extension global** (`packages/db/src/tenant-scope.ts`) — **scope dividido por tipo de modelo (v6.2)**:
  ...
  - **Status v7.6**: PR5a optó por **scoping manual per-service** en vez de extension global — cada service que toca tablas tenant-scoped escribe `where: { tenantId }` explícito. Trade-off: más boilerplate pero más debuggable que magic auto-injection. La extension queda como mejora opcional; revisitar si emerge dolor de scaling con muchos services nuevos.
- [ ] **B1.14.** **ESLint rule custom** (`packages/config-eslint/rules/no-unscoped-prisma.js`) — **whitelist por archivo exacto + helper único (v6.2)**:
  ...
  - **Status v7.6**: depende de B1.13 (la extension global es prerequisito conceptual de la regla). Postponed mientras B1.13 también lo está. Mientras tanto, code review humano + test suite de aislamiento son la barrera contra regresiones de scope.
...
**Criterio de salida B.1:** `Tenant` existe con `clerk_org_id` real de EscortsHub, Clerk Orgs sincroniza vía webhook, `publicMetadata.tenant_id` escrito en Clerk, **`TenantContextGuard` resuelve `tenantId` por una de las 2 vías de B1.2(a)** (Vía A claim del JWT si se hizo upgrade plan, o Vía B lookup server-side via Clerk SDK / tabla local `tenants` — ambas opciones aceptables), Prisma extension filtra las operaciones no-únicas, ESLint previene regresiones, todos los datos existentes quedan scopeados, tests de aislamiento básicos verdes.
...
- [x] **B2.1.** Refactor `SystemPromptService.buildAgentSystemPrompt(aiAgentId, ctx)` con composición dinámica — completado 2026-05-04 en 4 commits (`6cc79b8` data layer, `d343cd1` interfaces+wiring, `fc15a0a` 10 layer builders, `5434cf2` callers+cleanup EscortsHub). Session context cache expandido a `{ tenantId, aiAgentId }` (1 query). `AIOrchestratorService` y `ResponseGenerator` delegan al prompt dinámico. `getFormatInstructions` limpio de hardcoded EscortsHub. 9 tests nuevos en `SystemPromptService.spec.ts`. Fallback legacy cuando `aiAgentId` es null
- [x] **B2.2+B2.3.** Knowledge retrieval scoped por tenant+agent — completado 2026-05-04 commit `56d3133`. `getKnowledgeBase(category, {tenantId, agentId})` y `searchKnowledgeBase(query, {tenantId, agentId})` añaden `WHERE tenant_id AND agent_id` cuando presentes. `KnowledgeRetriever.retrieve(msg, intent, {tenantId, aiAgentId})` propaga opts a ambos métodos. `AIThinkingService.performKnowledgeRetrieval` pasa `enrichedContext.tenantId/aiAgentId`. Backward compatible (opts optional)
- [x] **B2.4+B2.5.** Pipeline threading + hardening — absorbidos progresivamente por B2.0 commit 2 (`ec928e2`), B2.1 commit `d343cd1`, y hardening commit (2026-05-04). Estado final:
  - `MessageHandler.processMessageWithAI` resuelve `{ tenantId, aiAgentId }` via `getSessionContext()` (1 query) y pasa ambos en `MessageContext`
  - `AIThinkingService.processWithThinking` recibe `aiAgentId` via context; `performKnowledgeRetrieval` lo propaga a `KnowledgeRetriever.retrieve(msg, intent, {tenantId, aiAgentId})`
...
- [ ] **C1.** Monitor de RAM por proceso (Winston alerta + webhook) cuando uso >80% del VPS. **Degradado a observabilidad genérica opcional** tras la decisión Baileys (2026-08-25): deja de ser crítico cuando desaparece Chromium
...
- [ ] ~~**C7.** Restart automático programado de cada worker cada 24–48h (workaround de memory leaks wwebjs) con drenado graceful~~ — **SUPERSEDED por la migración a Baileys (2026-08-25)**. No implementar: es un workaround de Chromium y sería trabajo tirado en el cutover
- [ ] ~~**C8.** Healthcheck específico: detectar "detached frame" errors y reiniciar sesión afectada~~ — **SUPERSEDED por la migración a Baileys (2026-08-25)**. Los `detached frame` son un fallo de Puppeteer; sin navegador no existen
...
| Memory leak de whatsapp-web.js en runtime largo                                                        | **Alta**         | Alto        | **Se elimina con la migración a Baileys (acordada 2026-08-25, no ejecutada).** Hasta el cutover: vigilar RAM manualmente. C7/C8 quedan superseded — no invertir en ellos                                                                                                                                                     |
...
| JWT de Clerk no incluye `tenant_id` por plan/template mal configurado o webhook de sync falló          | Media            | Medio       | **Mitigado por defecto via Vía B de B1.2(a)**: el `TenantContextGuard` no depende del claim — usa `orgId` built-in + lookup local `tenants`. Riesgo solo aplica si se elige Vía A (claim) y/o B1.2(b) JWT Template "supabase" para RLS directa, donde sí es prerequisito. Test e2e en B3.4 valida claim antes de activar RLS |
...
| 2026-08-25 | v7.11   | Eduard S. + Claude + Codex (5 rondas de review)          | **PR #12 + decisión de motor WhatsApp.** (1) **Decisión: migración directa a Baileys**, motor único, sin adapter dual — el techo de 5–8 sesiones es de Chromium y la fase piloto es la ventana barata. C7/C8 quedan superseded; `HANDOFF-BUMP-WAWEB.md` cerrado como WONTFIX. (2) **R1 reclasificado a CONDICIONADO**: Cloud API es incompatible con el onboarding QR de número propio; solo reabrible vía Coexistence si los clientes usan Business App. (3) **PR #12 (`fix/proactive-consent-and-tenant-scope`)**: cierra escritura y lectura cross-tenant en `bulk-update-whatsapp` (service-role sin filtro de tenant); añade gate de consentimiento al bulk proactivo, que enviaba sin comprobar `whatsapp_authorized`; alinea los 4 filtros de UI que trataban `!== false` como autorizado siendo el campo nullable; exige conversación INBOUND viva (`PROACTIVE_INBOUND_WINDOW_DAYS`, 30 por defecto, tope 90) acotada por tenant **y sesión**; **resuelve T3** (epoch en segundos leído como ms → `created_at` en 1970), que había dejado de ser inocuo al construir la ventana encima. Verificado: tsc limpio en los 3 apps, jest 71/71 (ws) y 61/61 (api). (4) Pendientes anotados en `PREEXISTING-ISSUES.md` § PR #12: reparar filas de 1970, rutas genéricas de envío sin gate, `getUserStats` sin tenant. **Nota**: la cabecera declaraba v7.10 pero el registro saltaba de v7.5 a esta entrada — las entradas v7.6–v7.10 no están documentadas; reconciliar quien tenga el contexto                                                                                                                                                                                     |
...
- **R1 — WhatsApp Business API oficial (Meta Cloud API)**: **CONDICIONADO, no planificado** (revisado 2026-08-25). Incompatible con el onboarding actual: el cliente conecta **su propio número** por QR, el que ya usa con sus contactos, y Cloud API exige número dedicado. Meta ofrece _Coexistence_ vía Embedded Signup, que permite conservar número y app — pero solo para cuentas de **WhatsApp Business App**, no WhatsApp personal, con límites de elegibilidad por cuenta y región más verificación y App Review. **Reabrir solo si** se comprueba que los clientes usan Business App y son elegibles, o si el producto pasa a ofrecer número propio. La migración de escala que sí se ejecuta es a Baileys (ver cabecera).
